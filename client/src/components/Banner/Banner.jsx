            ...
            ' />
                <div className=' max-w-6xl mx-auto flex flex-col md:flex-row items-center'>
                    ...
                    <div className=' flex-1 space-y-8 relative md:pr-8 lg:pr-19 text-center md:text-left'>
                        ...
                        <p className=' text-lg md:text-lg lg:text-xl font-playfair italic sm:text-amber-100
                         max-w-xl opacity-90 mx-auto md:mx-0  '>
                            Best cooks and bext delivery guys all at you service.
                            hot tasty food will reach you in 60 minutes.
                        </p>
                        <form onSubmit={handleSearch} className=' relative max-w-2xl mx-auto md:mx-0 group'>
                            <div className=' relative flex items-center bg-amber-900/30 rounded-xl 
                             border-2 border-amber-500/30 shadow-2xl hover:bg-amber-400/50 transition-all 
                             duration-300 '>
                                <div className=' pl-6 pr-3 py-4 '>
                                    <FaSearch className=' text-xl text-amber-400/80' />
                                </div>
                                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                                    placeholder='Discover your next fevorite meal...'
                                    className=' w-full py-4 pr-6 bg-transparent outline-none placeholder-amber-200/70 text-lg 
                                    font-medium tracking-wide '/>
                                <button type='submit' className=' mr-4 px-6 py-3 bg-gradient-to-r from-amber-400 to bg-amber-300 
                        ...
                        <div className=' flex flex-wrap gap-4 justify-center md:justify-start mt-6 '>
                            ...
                            <button onClick={() => setShowVideo(true)} className=' group flex items-center gap-3 bg-gradient-to-r from-amber-400 
                             to-amber-300 hover:from-amber-300 hover:to-amber-200 px-6 py-3 rounded-xl transition-all duration-300
                              shadow-lg hover:shadow-amber-300/30 '>
                    ...
                    {/*  Right Img Container With Orbital Images */}
                    <div className=' flex-1 relative group mt-8 md:mt-0 min-h-[300px] sm:min-h-[400px] '>
                        ...
                        <div className=' relative rounded-full p-1 bg-gradient-to-br from-amber-700 via-amber-800 to-amber-400 
                         shadow-2xl z-20 w-[250px] xs:w-[300px] sm:w-[350px] h-[250px] xs:h-[300px] sm:h-[350px] mx-auto '>
                            <img src={bannerImage} alt="Banner Image" className=' rounded-full border-4 xs:border-8 border-amber-900/50 
                        ...
                        {orbitImages.map((imgSrc, index) => (
                            <div key={index} className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 
                                ...
                                <img src={imgSrc} alt={`Orbiting ${index + 1}`}
                                    className=' w-full rounded-full border-amber-500/30 shadow-lg bg-amber-900/20 p-1 object-cover ' />
                            </div>
                        ))}
                    </div>
                </div>
